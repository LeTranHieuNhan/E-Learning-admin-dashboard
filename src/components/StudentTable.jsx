...
import { fetchRoles } from "../redux/actions/roleActions"; // Ensure you import the fetchRoles action
import StudentCardView from "./StudentCardView"; // Import the new StudentCardView component
...
const StudentTable = ({ students = null }) => {
    ...
    const [viewMode, setViewMode] = useState("table"); // New state to toggle between table and card view
    ...
    const handleEditUser = (student) => {
        const updatedStudent = {
            ...student,
            id: studentToEdit.id, // Ensure ID is preserved for update
            role: student.role,
        };
        console.log(updatedStudent);
        dispatch(updateUser(updatedStudent.id, updatedStudent, student.role.id));
            ...
            <div className="mb-4 flex justify-between items-center">
                ...
                <div>
                    <button
                        ...
                        className={`mr-4 ${viewMode === "table" ? "font-bold" : ""}`}
                    >
                        Table View
                    </button>
                    <button
                        onClick={() => setViewMode("card")}
                        className={`${viewMode === "card" ? "font-bold" : ""}`}
                    >
                        Card View
                    </button>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        {displayedStudents.map((student) => (
                            <tr key={student?.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    {new Date(student?.joinedAt).toLocaleDateString('en-us', { year: 'numeric', month: 'long', day: 'numeric' })}
                                </td>
